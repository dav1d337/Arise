...
class DeviceUuidFactory(val context: Context) {
    ...
    @Volatile protected var uuid: UUID? = null
     ...
     * The UUID is generated by using ANDROID_ID as the base key if appropriate, falling back on TelephonyManager.getDeviceID() if ANDROID_ID is known to be incorrect, and finally falling back on a
     * random UUID that's persisted to SharedPreferences if getDeviceID() does not return a usable value.

     * In some rare circumstances, this ID may change. In particular, if the device is factory reset a new device ID may be generated. In addition, if a user upgrades their phone from certain buggy
     * implementations of Android 2.2 to a newer, non-buggy version of Android, the device ID may change. Or, if a user uninstalls your app on a device that has neither a proper Android ID nor a
    ...
    val deviceIdString: String
        get() = this.deviceUuid.toString().replace("-", "")
